      ...
      if (m_pri_awa >= 4 || m_pri_twa >= 5 || m_pri_wdn >= 5) {
        if (m_nmea0183.Parse()) {
          if (m_nmea0183.Mwv.IsDataValid == NTrue) {
            ...
            } else if (m_nmea0183.Mwv.Reference == "T" &&
...
void dashboard_pi::SetPositionFixEx(PlugIn_Position_Fix_Ex &pfix) {
  ...
  if (m_pri_cogsog >= 1) {
    ...
    {
      devCOG = m_cog_filter.Filter(pfix.Cog);
      devSOG = m_sog_filter.Filter(pfix.Sog);
    }
...
void DashboardPreferencesDialog::SaveDashboardConfig() {
  ...
  // Hakan Momo Dev table
  b_IsDeviation = m_pSetDeviationBtn->IsChecked();
#ifdef __WXMSW__
